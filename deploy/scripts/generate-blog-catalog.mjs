...
// The slug table the edge function needs, and the only place it can come from.
//
// `/{lang}/blog/article.html?post={slug}` is the journal's retired
// client-side address, and netlify/edge-functions/legacy-article-url.ts turns
// it into the pre-rendered page. It used to do that by pasting the slug
// straight into the new path, which is right exactly once per article: the
// slugs are localized. The shell page had one slug per *article*, so a link
// that has been sitting in somebody's bookmarks since then reads
//
//     /pt/blog/article.html?post=slow-money-system
//
// and the Portuguese article is at /pt/blog/sistema-dinheiro-lento/. The old
// redirect sent that reader to /pt/blog/slow-money-system/, which is the 404
// Search Console reported. Every article has the same hole in two of its three
// languages; only the language whose slug happens to match the translation key
// ever resolved.
//
// So the mapping has to be looked up rather than computed, and the front
// matter's `translation_key` is what ties the three translations together. The
// table is keyed by every handle a legacy URL could carry - each language's
// slug and the translation key itself - because the shell page was live in all
// three languages and there is no record of which slug a given link picked up.
//
// Written as a module rather than JSON so Deno needs no import assertion, and
// generated rather than committed by hand because an article published in the
// content studio commits Markdown and nothing else. It lands in a `lib`
// subdirectory: Netlify reads `netlify/edge-functions/<name>.ts`,
// `<name>/index.ts` and `<name>/<name>.ts` as functions, and this is none of
// those, so it is a module the functions import and not a function itself.
const slugHandles = new Map();
for (const article of catalog) {
  const handles = new Set([article.translationKey, ...Object.values(article.translations)]);
  for (const handle of handles) slugHandles.set(handle, article.translations);
}

const edgeLibrary = path.join(root, 'netlify', 'edge-functions', 'lib');
await fs.mkdir(edgeLibrary, { recursive: true });
await fs.writeFile(
  path.join(edgeLibrary, 'article-slugs.ts'),
  [
    ...
    'export const ARTICLE_SLUGS: Record<string, Partial<Record<string, string>>> = '
      + `${JSON.stringify(Object.fromEntries([...slugHandles].sort(([a], [b]) => a.localeCompare(b))), null, 2)};`,
    ''
  ].join('\n')
);
